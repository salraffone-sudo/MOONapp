...
4. **Done.** Vercel gives you a URL like `moonapp.vercel.app`. Add it
...
- **Events list now spans 12 months uniformly.** Previously the header said
  "next 60 days" but eclipses and planet oppositions were deliberately shown
  regardless of distance, so a Saturn opposition 18 months out would appear
  in a "60 day" list — confusing, not actually a sort bug. Every event type
  now respects the same 12-month window and the header matches.
- **Planet visual, left of the moon** (`components/PlanetGlyph.js`): shows the
  top 2–3 brightest currently-visible planets, in roughly natural color.
  Sizing is a compressed relative scale, not true apparent angular size —
  at real apparent size every planet is an indistinguishable point of light,
  so this trades strict accuracy for something actually recognizable at a
  glance. Saturn gets a simplified ring, Jupiter gets faint bands, purely for
  identifiability. Adjust `PLANET_VISUALS` in that file to retune colors/sizes.
- **Eclipse alert, right of the moon** (`components/EclipseBadge.js`): only
  appears when a lunar or solar eclipse is both within 7 days AND actually
  visible from your location (moon above horizon for lunar; a real local
  circumstance for solar). Lunar shows a blood-red disc, solar shows a
